...
import { INCOMING_UPLOADS_PATH } from "../fileOperations/uploadStorage";
...
const receiveImage = multer({
  storage: multer.diskStorage({
    destination: (_req, _file, callback) => {
      mkdir(INCOMING_UPLOADS_PATH, { recursive: true }).then(
        () => callback(null, INCOMING_UPLOADS_PATH),
        (error: Error) => callback(error, INCOMING_UPLOADS_PATH),
      );
    },
  }),
...
const receivePostImage = (req: Request, res: Response) =>
  new Promise<ReceivedPostImage | undefined>((resolve, reject) => {
    void receiveImage(req, res, (error: unknown) => {
...
export const withReceivedPostImage = async <Result>(
  ...
  const image = await receivePostImage(req, res);
