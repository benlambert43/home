This post has no images at all. It walks through the Markdown a post can use.

# A first level heading

## A second level heading

### A third level heading

#### A fourth level heading

##### A fifth level heading

###### A sixth level heading

## Emphasis

Text can be **bold**, _italic_, **_bold and italic_**, or ~~struck through~~. It can hold `inline code`, and inline code can hold a backtick if it is fenced with two: ``a `backtick` inside``.

## Links

- An [internal link](/blog) stays in the same tab.
- An [external link](https://example.com) opens a new one.
- A [link with a title](https://example.com "The title shows on hover") has a tooltip.
- A [mailto link](mailto:hello@example.com) starts an email.
- A bare address, https://example.com/bare, is linked on its own.
- So is one in angle brackets: <https://example.com/angle>.
- A [link with an unsafe scheme](javascript:alert) is shown as plain text.

## Line breaks

A backslash at the end of a line\
breaks it, and so do two trailing spaces  
at the end of this one.

## Lists

- An unordered item
- Another unordered item
  - A nested item
    - A deeper nested item
- Back at the top level

1. An ordered item
2. A second ordered item
   1. A nested ordered item
   2. Another nested ordered item
3. A third ordered item

An ordered list does not have to start at one:

7. This list starts at seven
8. And carries on from there

- [x] A finished task
- [ ] An unfinished task
- [x] Another finished task

## Quotes

> A block quotation, for when someone else said it better.
>
> It can run to a second paragraph, hold **emphasis**, and hold a list:
>
> - one
> - two
>
> > It can even hold another quotation.

## Tables

| Left aligned | Centred | Right aligned |
| :----------- | :-----: | ------------: |
| apples       |   red   |          1.20 |
| bananas      | yellow  |          0.45 |
| `code`       | **yes** |        _also_ |

## Escapes

Characters that would mean something can be escaped: \*not italic\*, \_not italic\_, \`not code\`, \# not a heading, and 1\. not a list.

---

Three dashes make a horizontal rule, like the one above.
